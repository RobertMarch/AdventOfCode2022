...
fn simulate_rope(file: &String, rope_length: usize) -> String {
    let directions: HashMap<char, (isize, isize)> = HashMap::from([
        ('R', (1, 0)),
        ('L', (-1, 0)),
        ('U', (0, 1)),
        ('D', (0, -1)),
    ...
    for line in file.lines() {
        ...
        for _ in 0..distance {
            ...
            for segment in 1..rope_length {
                let rope_x_length = rope_segments.get(segment - 1).unwrap().0 - rope_segments.get(segment).unwrap().0;
                let rope_y_length = rope_segments.get(segment - 1).unwrap().1 - rope_segments.get(segment).unwrap().1;
