...
fn solve(
    ...
    add_to_known_points(
        ...
        search_direction,
        &mut known_points,
        &mut neighbour_points,
        &grid,
    );

    while neighbour_points.len() > 0 {
        let next_point = neighbour_points
            .iter()
...
fn add_to_known_points(
    point: &Point,
    search_direction: isize,
    known_points: &mut DistanceToPoints,
    neighbour_points: &mut DistanceToPoints,
    grid: &Grid,
) {
    ...
    for neighbour in neighbours {
        ...
        if neighbour_points.contains_key(&neighbour)
            && neighbour_points.get(&neighbour).unwrap() < &distance_to_point
        {
            continue;
        }
